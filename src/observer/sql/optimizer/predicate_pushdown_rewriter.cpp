...
RC PredicatePushdownRewriter::rewrite(std::shared_ptr<LogicalOperator> &oper, bool &change_made)
{
  RC rc = RC::SUCCESS;
  ...
  auto &child_oper = oper->children().front();
  ...
  auto &predicate_oper_exprs = oper->expressions();
  if (predicate_oper_exprs.size() != 1) {
    return rc;
  }

  auto                                    &predicate_expr = predicate_oper_exprs.front();
  ...
  if (!predicate_expr || is_empty_predicate(predicate_expr)) {
    ...
    predicate_expr = std::shared_ptr<Expression>(new ValueExpr(value));
  }
...
bool PredicatePushdownRewriter::is_empty_predicate(std::shared_ptr<Expression> &expr)
{
  bool bool_ret = false;
  if (!expr) {
    return true;
    ...
    std::shared_ptr<Expression> &expr, std::vector<std::shared_ptr<Expression>> &pushdown_exprs)
{
  ...
  if (expr->type() == ExprType::CONJUNCTION) {
    ...
    auto &child_exprs = conjunction_expr->children();
