...
RC try_to_get_bool_constant(std::shared_ptr<Expression> &expr, bool &constant_value)
{
...
RC ConjunctionSimplificationRule::rewrite(std::shared_ptr<Expression> &expr, bool &change_made)
{
  RC rc = RC::SUCCESS;
  ...
  for (auto iter = child_exprs.begin(); iter != child_exprs.end();) {
    ...
    if (conjunction_expr->conjunction_type() == ConjunctionExpr::Type::AND) {
      if (constant_value == true) {
        ...
        // always be false
        std::shared_ptr<Expression> child_expr = std::move(child_exprs.front());
        child_exprs.clear();
