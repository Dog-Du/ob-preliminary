...
RC ExpressionRewriter::rewrite(shared_ptr<LogicalOperator> &oper, bool &change_made)
{
  RC rc = RC::SUCCESS;

  bool sub_change_made = false;

  auto &expressions = oper->expressions();
...
RC ExpressionRewriter::rewrite_expression(shared_ptr<Expression> &expr, bool &change_made)
{
  RC rc = RC::SUCCESS;

  change_made = false;
  for (auto &rule : expr_rewrite_rules_) {
    bool sub_change_made = false;
  ...
  switch (expr->type()) {
    ...
    case ExprType::CAST: {
      auto &child_expr = (static_cast<CastExpr *>(expr.get()))->child();
