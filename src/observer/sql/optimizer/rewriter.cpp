...
RC Rewriter::rewrite(std::shared_ptr<LogicalOperator> &oper, bool &change_made)
{
  RC rc = RC::SUCCESS;

  change_made = false;
  for (std::shared_ptr<RewriteRule> &rule : rewrite_rules_) {
  ...
  std::vector<std::shared_ptr<LogicalOperator>> &child_opers = oper->children();
