...
RC PredicateRewriteRule::rewrite(std::shared_ptr<LogicalOperator> &oper, bool &change_made)
{
  ...
  auto &expressions = child_oper->expressions();
  ...
  if (true == bool_value) {
    std::vector<std::shared_ptr<LogicalOperator>> grand_child_opers;
