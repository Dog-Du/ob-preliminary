...
public:
  CreateTableStmt(const std::string &table_name, const std::vector<AttrInfoSqlNode> &attr_infos,
      StorageFormat storage_format, Db *db, SelectStmt *select_stmt)
      : db_(db),
  ...
  std::shared_ptr<SelectStmt>        &select_stmt() { return select_stmt_; }
...
private:
  ...
  std::shared_ptr<SelectStmt>  select_stmt_;
};