...
RC BplusTreeIndex::create(Table *table, const char *file_name,
    ...
    const std::vector<const FieldMeta *> &fields_meta, bool unique)
{
  ...
  Index::init(index_meta, fields_meta, unique);

  BufferPoolManager &bpm = table->db()->buffer_pool_manager();
  RC                 rc  = index_handler_.create(
      table->db()->log_handler(), bpm, file_name, fields_meta, unique);
  if (RC::SUCCESS != rc) {
    LOG_WARN("Failed to create index_handler");
    return rc;
  }
...
RC BplusTreeIndex::open(Table *table, const char *file_name,
    const IndexMeta                      &index_meta,
    const std::vector<const FieldMeta *> &field_meta, bool unique)
{
  if (inited_) {
    LOG_WARN("Failed to open index due to the index has been initedd before. file_name:%s");
    return RC::RECORD_OPENNED;
  }

  Index::init(index_meta, field_meta, unique);

  BufferPoolManager &bpm = table->db()->buffer_pool_manager();
  RC                 rc  = index_handler_.open(
      table->db()->log_handler(), bpm, file_name, field_meta, unique);
  if (RC::SUCCESS != rc) {
    LOG_WARN("Failed to open index_handler,");
    return rc;
  }
    ...
    bool right_inclusive)
{
  ...
  RC rc = index_scanner->open(left_key,
      left_len,
      left_inclusive,
      right_key,
      right_len,
...
RC BplusTreeIndexScanner::open(const char *left_key, int left_len,
    ...
    bool right_inclusive)
{
  return tree_scanner_.open(left_key,
      left_len,
      left_inclusive,
      right_key,
      right_len,
